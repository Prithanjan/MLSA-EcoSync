...
export default function StartScreen() {
  ...
  useEffect(() => {
    ...
    const initViewer = async () => {
      try {
        ...
        viewer.scene.globe.enableLighting = true; // Natural sunlight
  ...
  return (
    <div className="fixed inset-0 bg-[#040914] overflow-hidden">
      ...
      {/* Minimal UI matching Screenshot 2 */}
      <div 
        className={`absolute top-12 right-12 z-50 transition-all duration-700 ${showUI ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}
      >
        <div className="flex flex-col items-end">
          <div className="flex items-center gap-2 p-2 rounded-2xl bg-[#0B1526]/80 backdrop-blur-md border border-emerald-500/20 shadow-2xl">
            ...
            <input
              ...
              className="w-64 bg-[#142038]/50 border-none rounded-xl px-4 py-2.5 text-sm text-emerald-50 placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
            />
            
            <button
              onClick={handleSearch}
              disabled={isAnimating}
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-all text-sm font-medium border border-emerald-500/30"
            >
              {isAnimating ? (
          ...
          <div className="mt-3 mr-4 text-center">
             <p className="text-[10px] text-slate-500 font-medium tracking-wide">
               {error ? <span className="text-red-400">{error}</span> : "Click a glowing city \u00B7 or type any location"}
             </p>
          </div>
        </div>
      </div>
